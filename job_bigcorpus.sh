...
conda run -n cb pip install -q chatterbox-tts resemble-perth resemblyzer soundfile librosa praat-parselmouth peft yt-dlp openai-whisper "numpy<2" 2>&1 | tail -1
echo "CB_DEPS_DONE"
conda run -n cb python patch_chatterbox.py
echo "PATCH_STAGE_DONE"
conda run -n cb python download_corpus.py
