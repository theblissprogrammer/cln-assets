#!/bin/bash
set -x; echo "=== CB GENERATION-TIME DELIVERY SWEEP START $(date) ==="
...
conda run -n cb pip install -q chatterbox-tts resemble-perth soundfile librosa praat-parselmouth "numpy<2" 2>&1 | tail -2
echo "CB_DEPS_DONE"
conda run -n cb python gen_cb_sweep.py
