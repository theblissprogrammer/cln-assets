#!/bin/bash
set -x; echo "=== AR-BON (Arabic best-of-8) JOB START $(date) ==="
...
PY
mkdir -p out_ar
echo "=== Arabic best-of-8 (TOP-4 EN refs) ==="
python - <<'PY'
# -*- coding: utf-8 -*-
...
for sid,txt in ar:
    for j in range(8):
        try:
            out=m.inference(txt,"ar",gl,sp,temperature=0.75,enable_text_splitting=True)
            sf.write(f"out_ar/{sid}_c{j}.wav", out["wav"],24000); print("GEN",sid,j,flush=True)
        except Exception as e: print("GENERR",sid,j,repr(e),flush=True)
PY
echo "=== MEASURE Arabic best-of-8 vs her EN centroid ==="
python measure_bon.py out_ar
echo "=== JOB DONE $(date) ==="
